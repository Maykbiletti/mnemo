...
// --------------------------------------------------------------------------
// Promise discovery: scan recent agent messages for commit-phrases,
...
function discoverPromises({ actor = (process.env.MNEMO_AGENT_NAME || process.env.MNEMO_AGENT || "agent"), days = 14 }) {
...
function scanPromise(p) {
  ...
  const actor = p.actor || process.env.MNEMO_AGENT_NAME || process.env.MNEMO_AGENT || "agent";
...
function sweep() {
  const disc = discoverPromises({ actor: process.env.MNEMO_AGENT_NAME || process.env.MNEMO_AGENT || "agent", days: 14 });
