#!/usr/bin/env node
/**
 * agent_hand/run.js — entry point for the agent_hand skill.
 *
 * V1 just emits the prompt block the agent should hand to local runtime on the
...
(async () => {
  ...
  const block = [
    ...
    prompt.trim(),
    "",
    "After completion: report back via Mnemo `mem_add({kind:'agent_run',importance:7,...})`.",
  ].join("\n");
  ...
  const result = {
    ...
    notes: mode === "manual_paste"
      ? "mnemo-pc not paired — surface prepared_block to owner, ask them to paste into local runtime CLI"
