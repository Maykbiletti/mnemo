 ...
 *
 * Sources:
 *   1. chat.json (Telegram in/out 2026-03-23 → 2026-04-05 + Dieter outbound to today)
 ...
 *   6. .claude/projects/<sid>.jsonl (current + recent session conversations)
 *
  ...
  const tx = db.transaction(() => {
    for (const f of fs.readdirSync(dir)) {
      ...
      const ok = tryInsert(stmt, {
        ...
        occurred_at: occurred,
        actor: kind === "scar" ? "Dieter" : kind === "dream" ? "Dieter" : "system",
        topic: null,
    ...
    try {
      ...
      try {
        db.prepare(`INSERT OR IGNORE INTO session (id, started_at, jsonl_path, agent) VALUES (?,?,?,?)`)
          .run(sessionId, firstAt || nowIso(), fp, "dieter");
