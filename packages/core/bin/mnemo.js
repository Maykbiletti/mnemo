...
Env vars (all optional):
  ...
  MNEMO_OWNER_NAME          name of the agent's owner
  MNEMO_DEFAULT_SCOPE       private facts/project-rules scope
  MNEMO_DEFAULT_AGENT       primary agent name
  MNEMO_FACTS_DIR           directory for private facts JSON files
...
if (cmd === "pack" && args[1] === "apply" && args[2]) {
  ...
  // Try workspace packs first, then legacy package locations, then node_modules.
  const workspacePack = path.join(process.cwd(), "packs", packName, "pack.js");
  const local = path.join(process.cwd(), "packages", packName, "pack.js");
  const coreRelative = path.join(ROOT, "..", "..", "packs", packName, "pack.js");
  const installed = path.join(process.cwd(), "node_modules", `@mnemo/${packName}`, "pack.js");
  const candidates = [workspacePack, coreRelative, local, installed];
