...
#
# Registers each agent with the hub, polls for briefs, and pastes them into the
# matching Windows Terminal tab via clipboard + Ctrl+V + Enter.
#
# Set each Windows Terminal tab title to the agent id. Example:
# "agent-a" / "agent-b".
#
# Usage:
...
function Send-ToWindow {
  ...
  $hWnd = Find-WindowByTitle -Needle "(?i)$([regex]::Escape($Title))"
