...
**Provider prompt-cache TTL:** many runtimes cache prompts for a short window. Plan pruning passes around that TTL — don't prune mid-window if you're going to need the same context within the cache window.

**Implementation:** done by your agent client or custom loop — Mnemo doesn't manage your live context. Mnemo gives you `mem_recall` to fetch back what was pruned.
